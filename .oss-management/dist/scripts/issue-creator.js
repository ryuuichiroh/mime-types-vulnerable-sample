...
exports.generateReviewIssueForm = generateReviewIssueForm;
exports.generateApprovalIssue = generateApprovalIssue;
 ...
 */
function generateReviewIssueForm(version, diffs, guidelinesMap, sbomArtifactUrl) {
    const issueForm = {
        name: 'OSS利用見直しタスク',
 ...
 */
function generateApprovalIssue(version, reviewResults, sbomArtifactUrl, reviewResultsArtifactUrl) {
 ...
 */
async function createGitHubIssue(title, body, labels, assignees) {
    try {
...
async function main() {
    ...
    try {
        if (command === 'review') {
            ...
            // Build guidelines map from YAML file
            const guideProvider = new license_guide_provider_1.LicenseGuideProvider(guidelinesYamlPath);
            ...
            if (process.env.GITHUB_ACTIONS === 'true') {
                ...
                const issueNumber = await createGitHubIssue(title, issueFormYaml, ['oss-review'], assignee ? [assignee] : undefined);
            ...
            const reviewResults = reviewResultsData.results || reviewResultsData;
            ...
            if (process.env.GITHUB_ACTIONS === 'true') {
                ...
                const issueNumber = await createGitHubIssue(title, issueMarkdown, ['oss-approval'], assignee ? [assignee] : undefined);
